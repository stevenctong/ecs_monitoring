...
def main():
    ...
    target_name = "Node"
    target_url = "/dashboard/zones/localzone/nodes"
    ...
    req = req.json()
    nodes = req['_embedded']['_instances']
    # nodes is now a list of nodes

    # Using 'req' so we can re-use code without changing references
    for req in nodes:
        ...
        for field in req:
            if type(req[field]) is str:
                try:
                    ecsdata[node_name][field] = float(req[field])
                except:
                    pass
                ...
                get_summary_data(field=field, summary_dict=req[field], \
                                 ...
                                 summary_values=ecsdata_summary[node_name])
